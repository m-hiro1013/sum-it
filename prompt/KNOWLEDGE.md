...
### T104: Prompt Caching 最適レイヤリング 🆕
- キャッシュを最大限活用するためには、変動の少ない大規模コンテキスト（whiteboard等）をメッセージ群の最上位（先頭）に配置し、プロンプトの残りの部分をその後に続けることが重要。Claudeでは明示的な `cache_control` を、GeminiやOpenAIでは構造的な先頭一致を意識することで劇的なコスト削減（最大90%）が可能になる。
