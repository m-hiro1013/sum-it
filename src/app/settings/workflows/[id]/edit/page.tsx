...
import { updateMeetingWorkflow, getMeetingWorkflow, getAgents } from "@/lib/firestore";
...
export default function EditWorkflowPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (workflowData) {
                ...
                setStartPrompt(workflowData.start_prompt);
                setEndPrompt(workflowData.end_prompt);
                setSelectedAgentIds(workflowData.agent_ids);
                setSteps(workflowData.steps);
                setIsActive(workflowData.is_active);
            } else {
                alert("ワークフローが見つからなかったよ😭");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const workflowData: Partial<MeetingWorkflowInput> = {
                name,
                description,
            ...
            await updateMeetingWorkflow(id, workflowData);
            router.push("/settings/workflows");
        } catch (error) {
            console.error(error);
            alert("更新に失敗しちゃった😭");
            ...
            <main className="container mx-auto py-12 px-4 max-w-4xl">
                ...
                <div className="mb-12">
                    <h1 className="text-4xl font-black tracking-tight flex items-center gap-3">
                        <Zap className="text-yellow-500" size={36} /> Edit Workflow
                    </h1>
                    <p className="mt-3 text-gray-500 font-medium tracking-tight">ワークフローの内容を調整しましょう。💅✨</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="flex items-center justify-between pt-8">
                        ...
                        >
                            {isSubmitting ? <Loader2 className="animate-spin" /> : "UPDATE WORKFLOW"}
                        </Button>
                    </div>
                </form>
            </main>
